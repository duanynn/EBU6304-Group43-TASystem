...
<main class="app-main talent-main">
    <% {
        Object profileMsg = session.getAttribute("taProfileMessage");
        if (profileMsg instanceof String s && !s.isBlank()) {
            String profileType = "info";
            Object typeObj = session.getAttribute("taProfileMessageType");
            if (typeObj instanceof String t && !t.isBlank()) profileType = t;
            String profileAlert = "error".equals(profileType) ? "alert alert-error"
                    : ("success".equals(profileType) ? "alert alert-success" : "alert alert-info");
    %>
    <div class="<%= profileAlert %>" role="alert"><%= h(s) %></div>
    <% session.removeAttribute("taProfileMessage"); session.removeAttribute("taProfileMessageType"); }
        Object accountMsg = session.getAttribute("accountMessage");
        if (accountMsg instanceof String s && !s.isBlank()) {
            String accountType = "info";
            Object typeObj = session.getAttribute("accountMessageType");
            if (typeObj instanceof String t && !t.isBlank()) accountType = t;
            String accountAlert = "error".equals(accountType) ? "alert alert-error"
                    : ("success".equals(accountType) ? "alert alert-success" : "alert alert-info");
    %>
    <div class="<%= accountAlert %>" role="alert"><%= h(s) %></div>
    <% session.removeAttribute("accountMessage"); session.removeAttribute("accountMessageType"); }} %>
    ...
    (function () {
        ...
        if (profileForm) {
            profileForm.addEventListener('submit', function (evt) {
                var avatarInput = profileForm.querySelector('input[type="file"][name="avatarFile"]');
                if (avatarInput && avatarInput.files && avatarInput.files.length > 0
                        && window.UploadLimitsClient
                        && !window.UploadLimitsClient.checkAvatar(avatarInput.files[0])) {
                    evt.preventDefault();
                    return;
                }
